...
class AgentModelsConfig(BaseModel):
    ...
    ``extra="forbid"`` — 정의되지 않은 모델 슬롯(예: 제거된 ``reviewer_model``,
    ``scorer_model``, ``reflector_model``)이 yaml에 남아 있으면 silent 무시 대신
    명시적 에러로 사용자에게 마이그레이션을 알립니다.
    """
...
class AgentRagConfig(BaseModel):
    ...
    ``extra="forbid"`` — 제거된 필드(``ralph_loop_*``, ``quality_mode``,
    ``ultra_mode``, ``reflector_*``, ``review_work_*``, ``self_improvement_*`` 등)가
    yaml에 남아 있으면 silent 무시 대신 boot 시점에 명시적으로 거부합니다.
    """
    ...
    적용 대상 — Planner, Verifier 2곳.
    최종 답변 합성(``_stream_synthesis``)은 thinking 누적으로 스트리밍 레이턴시가
    폭증하는 문제가 실측되어 의도적으로 제외됩니다. ReAct AgentLoop / QA 생성 /
    Augmenter / 단순 RAG 경로는 JSON 파싱·포맷 라벨 안정성과 배치 처리량을
    위해 항상 ``think=False``로 고정됩니다(이 플래그의 영향을 받지 않습니다).
    ...
    def _apply_smart_mode(self) -> "AgentRagConfig":
        """smart_mode 프리셋을 개별 플래그에 cascade."""
        if self.smart_mode:
