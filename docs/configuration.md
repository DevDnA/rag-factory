...
| `smart_mode` | `bool` | `false` | **원클릭 프리셋** — `true`이면 IntentClassifier + Clarifier + Personas + Planner + Verifier + Legacy fallback + Session source reuse 자동 활성화 |
...
**참고**
- 모든 LLM 호출 컴포넌트(Planner / Verifier / Clarifier / IntentClassifier / Compressor)는 **never-raise 계약**을 따릅니다 — LLM 장애 시 안전한 기본값으로 fallback하여 agent 전체를 중단시키지 않습니다.
